...
def test_base():
    ...
    schema = get_schema(build_call_model(handler, caster_cls=PydanticSerializer, dependency_provider=Provider()))
...
class TestNoType:
    ...
    def test_no_type_embeded(self):
        ...
        schema = get_schema(build_call_model(handler, caster_cls=PydanticSerializer, dependency_provider=Provider()), embed=True)
...
class TestOneArgWithModel:
    ...
    def test_resolved_model(self):
        ...
        schema = get_schema(build_call_model(handler, caster_cls=PydanticSerializer, dependency_provider=Provider()), resolve_refs=True)
        assert schema == {
            "properties": {
                "a": {
                    "properties": {"a": {"title": "A", "type": "integer"}},
    ...
    def test_optional_embeded_model(self):
        ...
        schema = get_schema(build_call_model(handler, caster_cls=PydanticSerializer, dependency_provider=Provider()), resolve_refs=True, embed=True)
...
def test_depends():
    ...
    schema = get_schema(
        build_call_model(handler, extra_dependencies=(Depends(dep4),), caster_cls=PydanticSerializer, dependency_provider=Provider()),
        resolve_refs=True,
        embed=True,
    )
